...
public class Puzzle extends JPanel {
  ...
  // This Thread shuffles the Segments by switching one of the neighboring segments of the empty one
  // with the empty one
  Thread mix =
      new Thread(
          new Runnable() {
            public void run() {
              ...
              while (mixing) {
                ...
                for (Segment s : segments) {
                  if (s.getPosition().x == segments[8].getPosition().x + 1
                      && s.getPosition().y == segments[8].getPosition().y)
                    possibleMovements.add(s.getID());
                  if (s.getPosition().x == segments[8].getPosition().x - 1
                      && s.getPosition().y == segments[8].getPosition().y)
                    possibleMovements.add(s.getID());
                  if (s.getPosition().x == segments[8].getPosition().x
                      && s.getPosition().y == segments[8].getPosition().y - 1)
                    possibleMovements.add(s.getID());
                  if (s.getPosition().x == segments[8].getPosition().x
                      && s.getPosition().y == segments[8].getPosition().y + 1)
                    possibleMovements.add(s.getID());
    ...
    for (Segment s : segments) {
      if (s.hitten(e.getPoint())) {
        ...
        if (s.move(segments[8].getPosition())) {
          ...
          boolean done = true;
          for (int i = 0; i != 9; i++) {
            if (segments[i].getPosition().x == ((i <= 2) ? i : (i <= 5) ? (i - 3) : (i - 6))
                && segments[i].getPosition().y == (int) Math.ceil((i / 3))) {
